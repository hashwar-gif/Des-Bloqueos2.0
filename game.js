...
const playlist = [
    { name: "Hashwar (Main)", file: "music/b)Hashwar❌ JDR-IA.wav" },
    { name: "$hash", file: "music/a)$hash❌ JDR-IA.wav" }
];
...
const charAbilities = {
    'pm': { name: 'VISOR TÁCTICO', icon: '🎯', cd: 720, dur: 180,            desc: 'Dron + visión térmica: balas +1 daño, enemigos marcados' },
    'policia_bol': { name: 'MACANAZO POLICIAL', icon: '🚓', cd: 900, dur: 300, desc: 'Macanazo + gas lacrimógeno + defensa +20' },
    'rebelde': { name: 'INCENDIO PROVOCADO', icon: '🔥', cd: 540, dur: 0,      desc: '5 molotovs crean MURO DE FUEGO cruzando la pantalla' },
    'dea': { name: 'DRON REAPER', icon: '🇺🇸', cd: 420, dur: 180,      desc: 'Dron Reaper bombardea enemigos con misiles teledirigidos' },
    'jara': { name: 'GRITO DE EUFORIA', icon: '🗣️', cd: 900, dur: 300,      desc: 'Grito de guerra: ondas de choque dañan y empujan enemigos' },
    'cholita': { name: 'FOGATA DE LA ABUELA', icon: '🪔', cd: 780, dur: 360,  desc: 'Fogata sagrada: cura + espíritus de fuego atacan' },
    'poncho_j': { name: 'TEJIDO SAGRADO', icon: '✨', cd: 1200, dur: 240,     desc: 'INVULNERABLE 4s + refleja daño + velocidad 2x' },
    'cob_j': { name: 'DINAMITA DE SOCAVÓN', icon: '💣', cd: 600, dur: 0,     desc: '3 dinamitas en cadena con explosión masiva' },
    'evo_j': { name: 'LLUVIA DE HOJAS DE COCA', icon: '🌿', cd: 720, dur: 360,     desc: 'Lluvia de hojas de coca: caen del cielo, los enemigos se drogan y reciben daño' },
    'rodrigo': { name: 'DECRETO SUPREMO', icon: '📜', cd: 1500, dur: 0,       desc: '8 bombas guiadas + suelta ítems de botín' },
    'perro': { name: 'CADENA MORTAL', icon: '☠️', cd: 600, dur: 180,         desc: 'Carga imparable + explosiones + fuego + invulnerable' },
    'joe': { name: 'CÓDIGO FUENTE', icon: '💻', cd: 600, dur: 180,            desc: 'Hackea enemigos: se atacan entre sí + ondas de hackeo' }
};
...
const systemStories = {
    ...
    end_good_chapare: { loc: "🏆 BOLIVIA LIBRE - EVO HA CAÍDO", txt: "Con apoyo aéreo de la DEA, las fuerzas del orden han logrado lo imposible. Evo Gonzalez ha sido capturado en su propio bastión. Las plantaciones de coca arden mientras los helicópteros barren la selva. La resistencia se desmorona. Los líderes de la rebelión son llevados ante la justicia. ¡CAYÓ EVO, CAYÓ EL BLOQUEO! Las rutas nacionales vibran de nuevo con el ir y venir de los camiones. El pueblo boliviano, cansado de la división, tiende puentes donde antes había trincheras. En cada plaza, en cada mercado, la gente celebra el fin del conflicto. La Paz despierta con los brazos abiertos. El diálogo ha vencido a la violencia. ¡FELICIDADES! DESBLOQUEASTE EL PAÍS. Bolivia entera respira una nueva esperanza bajo un solo cielo. ¡MISIÓN CUMPLIDA! Evo Gonzalez está bajo custodia y la nación entera celebra su liberación." }
};
...
window.closeShop = function() {
    ...
    const chapareChars = ['pm', 'policia_bol', 'rodrigo', 'rebelde', 'dea', 'jara'];
    const goesToChapare = chapareChars.includes(selectedChar);
    if (currentLevel < 5) {
        showStory(currentLevel + 1, () => {
            init(currentLevel + 1);
        });
    }
    else if (currentLevel === 5 && goesToChapare) {
        showEndingStory('chapare');
    }
    else if (currentLevel === 5 && !goesToChapare) {
        showStory(6, () => {
            init(6);
...
window.showEndingStory = function(type) {
    ...
    const chapareChars = ['pm', 'policia_bol', 'rodrigo', 'rebelde', 'dea', 'jara', 'joe'];
    const isGood = goodChars.includes(selectedChar);
    endingIsGood = isGood;
    const isChapareEnding = type === 'chapare' || (currentLevel >= 6 && chapareChars.includes(selectedChar));
    // Final combinado: si eres bueno y capturaste a Evo, mezcla ambos finales
    const isGoodChapare = isChapareEnding && isGood;
    const key = isGoodChapare ? 'end_good_chapare' : (isChapareEnding ? 'end_chapare' : (isGood ? 'end_good' : 'end_bad'));
...
window.startFinalCredits = function() {
    ...
    const chapareChars = ['pm', 'policia_bol', 'rodrigo', 'rebelde', 'dea', 'jara', 'joe'];
    const isGood = goodChars.includes(selectedChar);
    const isChapareEnding = window._endingType === 'chapare' || (currentLevel >= 6 && chapareChars.includes(selectedChar));
    const isGoodChapare = isChapareEnding && isGood;
    const endingTitle = isGoodChapare ? 'EVO HA CAÍDO ★ DESBLOQUEASTE EL PAÍS' : (isChapareEnding ? 'EVO HA CAÍDO' : (isGood ? 'DESBLOQUEASTE EL PAÍS' : 'BLOQUEASTE EL PAÍS'));
    const endingColor = isGoodChapare ? '#ffd700' : (isChapareEnding ? '#ffd700' : (isGood ? '#0f0' : '#f44'));
    const charNames = {
        'pm': 'POLICÍA MILITAR', 'policia_bol': 'POLICÍA BOLIVIANA', 'rodrigo': 'PRESIDENTE',
        'rebelde': 'REBELDE', 'dea': 'AGENTE DEA', 'jara': 'CAP. LARI',
...
function drawEndingScene(isGood, isChapare) {
    ...
    if (isChapare && isGood) {
        // FINAL COMBINADO — Captura de Evo + Bolivia Liberada
        let g = sctx.createLinearGradient(0,0,0,300);
        g.addColorStop(0, '#0a1a0a'); g.addColorStop(0.3, '#1a4a23'); g.addColorStop(0.6, '#e65100'); g.addColorStop(1, '#ffb300');
        sctx.fillStyle = g; sctx.fillRect(0,0,800,300);
        
        // Helos DEA
        for(let i=0; i<2; i++) {
            let hx = ((Date.now() * 0.03 + i * 250) % 900) - 50;
            sctx.fillStyle = '#222'; sctx.fillRect(hx, 30 + i*30, 35, 8);
            sctx.fillRect(hx+10, 23 + i*30, 15, 3);
            sctx.fillStyle = '#ffd700'; sctx.font = "bold 5px Arial"; sctx.fillText("DEA", hx+8, 36 + i*30);
        }
        
        // Selva con carretera abierta debajo
        sctx.fillStyle = '#1a4a14'; sctx.beginPath(); sctx.moveTo(-100, 300); sctx.lineTo(200, 160); sctx.lineTo(500, 200); sctx.lineTo(800, 150); sctx.lineTo(900, 300); sctx.fill();
        ...
        // Evo capturado
        sctx.fillStyle = '#000'; sctx.fillRect(300, 200, 30, 45);
        sctx.fillStyle = '#e8be94'; sctx.fillRect(305, 195, 20, 12);
        sctx.fillStyle = '#fff'; sctx.fillRect(303, 192, 24, 5);
        sctx.fillStyle = '#ff0'; sctx.font = "bold 8px Arial"; sctx.fillText("EVO", 306, 210);
        sctx.strokeStyle = '#ff0'; sctx.lineWidth = 2; sctx.strokeRect(290, 190, 50, 65);
        ...
        // Tráfico en carretera (Bolivia ya circula)
        for (let i = 0; i < 3; i++) {
        ...
        // Gente celebrando con banderas
        for (let i = 0; i < 4; i++) {
            let px = 450 + i * 90 + Math.sin(Date.now()*0.003 + i)*5;
        ...
        for (let i = 0; i < 4; i++) {
            ...
            sctx.fillStyle = '#5d4037'; sctx.fillRect(flagX + wave, 175, 3, 30);
            let colors = ['#f00', '#ff0', '#0a0'];
            colors.forEach((c, j) => { sctx.fillStyle = c; sctx.fillRect(flagX + wave, 175 + j*5, 20 + wave, 5); });
        }
        
        // Humo de plantaciones mezclado con amanecer
        for(let i=0; i<4; i++) {
            let t = Date.now()*0.003 + i*2;
            sctx.fillStyle = 'rgba(255,200,100,0.15)';
            sctx.beginPath(); sctx.arc(80 + i*150 + Math.sin(t)*20, 180 - Math.sin(t*0.5)*10, 25 + i*5, 0, 7); sctx.fill();
        }
        
        sctx.fillStyle = '#ffd700'; sctx.font = "bold 16px 'Black Ops One',system-ui";
        sctx.fillText("🏆 EVO CAÍDO — BOLIVIA LIBRE 🏆", 200, 30);
        sctx.fillStyle = '#fff'; sctx.font = "bold 9px Arial";
        sctx.fillText("LA NACIÓN UNIFICADA CELEBRA SU LIBERACIÓN", 230, 50);
    } else if (isChapare) {
...
function useAbility() {
    ...
    if (selectedChar === 'pm') {
        // PM - VISOR TÁCTICO: dron + modo visión térmica
        drones.push(new TactDrone(plyr.x + (plyr.f === 1 ? 60 : -60), plyr.y - 80));
        // Marca todos los enemigos visibles
        enms.forEach(e => { if (e.x > camX - 100 && e.x < camX + cvs.width + 100) { e._marked = true; if (e._origSp === undefined) e._origSp = e.sp; } });
        plyr.damageMod += 1; // +1 daño mientras dure
        abilActive = true;
        psnd('shoot');
    } else if (selectedChar === 'policia_bol') {
        // Policia - MACANAZO POLICIAL: macana + gas + defensa
        plyr.defense += 20;
        // Macanazo — golpea enemigos en un arco delante del policía
        let alcance = 80, anguloInicio = plyr.f === 1 ? -0.6 : Math.PI - 0.6, anguloFin = plyr.f === 1 ? 0.6 : Math.PI + 0.6;
        enms.forEach(e => {
            let dx = e.x - (plyr.x + 15), dy = e.y - (plyr.y + 25);
            let dist = Math.sqrt(dx*dx + dy*dy);
            if (dist < alcance) {
                let ang = Math.atan2(dy, dx);
                if (ang < 0) ang += Math.PI * 2;
                let aInicio = anguloInicio < 0 ? anguloInicio + Math.PI * 2 : anguloInicio;
                let aFin = anguloFin < 0 ? anguloFin + Math.PI * 2 : anguloFin;
                if (ang >= aInicio && ang <= aFin) {
                    e.hp -= 10;
                    e.x += (e.x - plyr.x) * 2;
                    for (let p = 0; p < 8; p++) parts.push(new Part(e.x + Math.sin(p*3)*10, e.y + Math.cos(p*2)*10, '#fff'));
                }
            }
        });
        // Arco visual de la macana
        for (let i = 0; i < 20; i++) {
            let t = i / 20;
            let ang = plyr.f === 1 ? -0.6 + t * 1.2 : Math.PI - 0.6 + t * 1.2;
            let px = plyr.x + 15 + Math.cos(ang) * 60;
            let py = plyr.y + 25 + Math.sin(ang) * 60;
            parts.push(new Part(px, py, i % 3 === 0 ? '#888' : '#555'));
        }
        // Gas lacrimógeno
        teargas.push(new TearGas(plyr.x + 15, plyr.y - 30));
        teargas.push(new TearGas(plyr.x - 20, plyr.y - 30));
        abilActive = true;
        psnd('boom');
    } else if (selectedChar === 'rebelde') {
        // Rebelde - INCENDIO PROVOCADO: muro de fuego cruza la pantalla
        let baseX = plyr.x - 200;
        for (let i = 0; i < 5; i++) {
            setTimeout(() => {
                if (!gover && !gwin) {
                    let tx = baseX + i * 120;
                    ...
                    // Fuego secundario se extiende
                    fires.push(new Fire(tx - 40));
                    fires.push(new Fire(tx + 40));
                    // Explosión masiva
                    for (let p = 0; p < 15; p++) parts.push(new Part(tx + Math.sin(p*3)*25, cvs.height - 30 + Math.cos(p*2)*15, ['#f40','#ff0','#f00','#fff'][p%4]));
                    // Daño + empuje
                    enms.forEach(e => {
                        if (Math.abs(e.x - tx) < 100) {
                            e.hp -= 8;
                            e.x += (e.x - tx) * 0.5;
        ...
        // Cholita - FOGATA DE LA ABUELA: fogata + espíritus de fuego
        let fx = plyr.x + (plyr.f === 1 ? 50 : -50);
        fogatas.push(new Fogata(fx, cvs.height - 35));
        // Espíritus de fuego (se crean periódicamente en update)
        if (!window._fogataEspiritus) window._fogataEspiritus = [];
        window._fogataEspiritus.push({ x: fx, y: cvs.height - 45, timer: 0 });
        for (let i = 0; i < 20; i++) parts.push(new Part(fx + Math.sin(i*3)*20, cvs.height - 40 + Math.cos(i*2)*12, '#ffaa00'));
        abilActive = true;
        psnd('boom');
    } else if (selectedChar === 'poncho_j') {
        // Poncho Rojo - TEJIDO SAGRADO: invulnerable + refleja + velocidad
        plyr._invulnerable = true;
        plyr.defense += 100; // básicamente invulnerable
        abilActive = true;
        psnd('collect');
    } else if (selectedChar === 'cob_j') {
        // COB - DINAMITA DE SOCAVÓN: 3 dinamitas en abanico con cadena
        for (let i = 0; i < 3; i++) {
            setTimeout(() => {
                if (!gover && !gwin) {
                    let spread = (i - 1) * 25;
                    let dx = plyr.x + (plyr.f === 1 ? 20 + spread : spread - 20);
                    let d = new Dyna(dx, plyr.y - 10, plyr.f);
                    d._chain = true; // marca para explosión en cadena
                    dynas.push(d);
                    psnd('boom');
                }
            }, i * 150);
        ...
        // Evo - LLUVIA DE HOJAS DE COCA: caen hojas del cielo, recuperan HP
        for (let i = 0; i < 25; i++) {
            let leaf = new CocaLeaf();
            leaf.y = -Math.random() * cvs.height;
            leaf.x = Math.random() * cvs.width;
            cocaLeaves.push(leaf);
        }
        abilActive = true;
        psnd('collect');
    } else if (selectedChar === 'dea') {
        // DEA - DRON REAPER: drone de ataque con misiles teledirigidos
        drones.push(new ReaperDrone(plyr.x, 30));
        abilActive = true;
    } else if (selectedChar === 'jara') {
        // LARI - GRITO DE EUFORIA: ondas de choque dañan y empujan enemigos
        // Explosión de sonido inicial — daña a TODOS en pantalla
        enms.forEach(e => {
            if (e.x > camX - 50 && e.x < camX + cvs.width + 50 && e.hp > 0) {
                e.hp -= 8;
                e.x += (e.x - plyr.x) * 0.5;
                for (let p = 0; p < 6; p++) parts.push(new Part(e.x + Math.sin(p*3)*10, e.y + Math.cos(p*2)*10, '#ffbb00'));
                if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
            }
        });
        ...
        for (let i = 0; i < 3; i++) {
            setTimeout(() => {
                if (!gover && !gwin) {
                    radioWaves.push(new RadioWave(plyr.x, plyr.y + 25));
                    for (let p = 0; p < 20; p++) parts.push(new Part(plyr.x + 15 + Math.sin(p*5)*i*20, plyr.y + 25 + Math.cos(p*3)*10, '#ffdd44'));
                }
            }, i * 80);
        }
        // Partículas de euforia alrededor del jugador
        for (let i = 0; i < 30; i++) {
            let ang = Math.random() * 7;
            let dist = Math.random() * 60;
            parts.push(new Part(plyr.x + 15 + Math.cos(ang) * dist, plyr.y + 25 + Math.sin(ang) * dist, '#ffdd44'));
        }
        abilActive = true;
    } else if (selectedChar === 'rodrigo') {
        // Rodrigo - DECRETO SUPREMO: bombas guiadas + botín
        const targetEnemies = enms.filter(e => e.x > camX - 50 && e.x < camX + cvs.width + 50 && e.hp > 0);
        const bombCount = 8;
        for (let i = 0; i < bombCount; i++) {
            setTimeout(() => {
                if (!gover && !gwin) {
                    // Bomba guiada hacia enemigos
                    let targetX = plyr.x + (Math.random() - 0.5) * 600;
                    if (targetEnemies.length > 0) {
                        let nearest = null, nearDist = 9999;
                        targetEnemies.forEach(e => {
                            let d = Math.abs(e.x - targetX);
                            if (d < nearDist) { nearDist = d; nearest = e; }
                        });
                        if (nearest) targetX = nearest.x;
                    }
                    fires.push(new Fire(targetX));
                    fires.push(new Fire(targetX - 30));
                    fires.push(new Fire(targetX + 30));
                    for (let p = 0; p < 20; p++) parts.push(new Part(targetX + Math.sin(p*4)*30, cvs.height - 20 + Math.cos(p*3)*15, ['#f44','#ff0','#f80','#fff'][p%4]));
                    enms.forEach(e => {
                    ...
                    // Suelta botín: dinero, balas, comida
                    if (i % 2 === 0) {
                        itms.push(new Item(targetX + Math.sin(i)*30, cvs.height - 90, i % 3 === 0 ? 'balas' : 'comida'));
                        totalMoney += 10;
                    }
                    psnd('boom');
                    updateUI();
        ...
        // Perro - CADENA MORTAL: carga imparable
        plyr._cargando = true;
        plyr._invulnerable = true;
        // Carga hacia adelante
        let chargeDir = plyr.f;
        let chargeDist = 300;
        let startX = plyr.x;
        let explosionInterval = setInterval(() => {
            if (gover || gwin || Math.abs(plyr.x - startX) > chargeDist) {
                clearInterval(explosionInterval);
                plyr._cargando = false;
                plyr._invulnerable = false;
                return;
            }
            // Explota todo a su paso
            plyr.x += chargeDir * 8;
            let ex = plyr.x;
            dynas.push(new Dyna(ex, plyr.y - 10, 0));
            fires.push(new Fire(ex));
            for (let p = 0; p < 8; p++) parts.push(new Part(ex + Math.sin(p*3)*12, plyr.y + Math.cos(p*2)*10, ['#f60','#ff0','#f00','#fff'][p%4]));
            enms.forEach(e => {
                if (Math.abs(e.x - ex) < 80) {
                    e.hp -= 10;
                    e.x += chargeDir * 15; // empujón
                    if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                }
            });
            psnd('boom');
        ...
        // Joe - CÓDIGO FUENTE: hackea enemigos, se atacan entre sí
        for (let i = 0; i < 8; i++) {
            setTimeout(() => {
                if (!gover && !gwin) {
                    ...
                    for(let p=0; p<6; p++) parts.push(new Part(wx+Math.sin(p*3)*15, wy+Math.cos(p*2)*10, '#00ff88'));
                    enms.forEach(e => {
                        if (Math.abs(e.x - wx) < 180 && Math.abs(e.y - wy) < 120) {
                            e.hp -= 3;
                            if (e._hackedByJoe === undefined) e._hackedByJoe = true;
                            for(let p=0; p<3; p++) parts.push(new Part(e.x+Math.sin(p)*8, e.y-15+Math.cos(p)*8, '#00ff88'));
                            if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                        }
                    });
                }
        ...
        // Hackea enemigos cercanos para que se ataquen entre sí
        enms.forEach(e => {
            if (Math.abs(e.x - plyr.x) < 300) {
                if (e._origSp === undefined) e._origSp = e.sp;
                e._hackedByJoe = true;
                e.sp = 0.2;
            }
        });
...
class CocaLeaf { constructor() { this.reset(); } reset() { this.x = Math.random() * cvs.width; this.y = Math.random() * -cvs.height - 50; this.v = 1 + Math.random() * 2; this.w = 6 + Math.random() * 4; this.h = 3 + Math.random() * 2; this.sway = Math.random() * 7; this.speed = 0.02 + Math.random() * 0.03; this.color = ['#3a7d2c', '#4a8c3c', '#2d6b1e', '#5a9c4a'][Math.floor(Math.random() * 4)]; } update() { this.y += this.v; this.x += Math.sin(gframe * this.speed + this.sway) * 1.2; if (this.y > cvs.height + 20) this.reset(); } draw() { ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(Math.sin(gframe * this.speed + this.sway) * 0.3); ctx.fillStyle = this.color; ctx.beginPath(); ctx.ellipse(0, 0, this.w / 2, this.h / 2, 0, 0, 7); ctx.fill(); ctx.strokeStyle = '#2a6a1a'; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(-this.w / 2 + 2, 0); ctx.lineTo(this.w / 2 - 2, 0); ctx.stroke(); ctx.restore(); } }
...
class MuroTactico {
    constructor(x, y) {
        ...
        this.tm = 300; this.a = true;
    }
    update() {
        this.tm--;
        if (this.tm <= 0) { this.a = false; return; }
        // Empuja enemigos hacia atrás
        enms.forEach(e => {
            if (Math.abs(e.x - this.x) < this.w && Math.abs(e.y - this.y) < this.h) {
                e.x += (e.x > this.x ? 3 : -3);
            }
        });
    }
    draw() {
        ...
        ctx.fillRect(sx - this.w/2, this.y - this.h, this.w, 4);
        ctx.fillRect(sx - this.w/2, this.y - this.h + 20, this.w, 3);
        ctx.fillRect(sx - this.w/2, this.y - 10, this.w, 4);
        // Símbolo policial
        ctx.fillStyle = '#fff';
        ctx.font = "bold 18px Arial";
        ctx.fillText("🚧", sx - 10, this.y - 25);
        // Resplandor
        ctx.fillStyle = `rgba(68, 136, 255, ${0.1 + Math.sin(gframe*0.05)*0.05})`;
        ctx.fillRect(sx - this.w/2 - 5, this.y - this.h - 5, this.w + 10, this.h + 10);
        ctx.restore();
    }
}
...
class ReaperDrone {
    ...
    update() {
        ...
        if (this.tm <= 0) { this.a = false; return; }
        this.atk++;
        // Sigue al jugador horizontalmente
        this.x += (plyr.x - this.x) * 0.03;
        // Busca enemigo más cercano en pantalla
        let nearest = null, nearDist = 9999;
        enms.forEach(e => {
            if (e.hp > 0 && e.x > camX - 50 && e.x < camX + cvs.width + 50) {
                let d = Math.abs(e.x - this.x);
                if (d < nearDist) { nearDist = d; nearest = e; }
            }
        });
        this.target = nearest;
        // Lanza misil cada 25 frames si hay objetivo
        if (this.target && this.atk % 25 === 0) {
            let bx = this.target.x;
            let by = cvs.height - 30;
            // Explosión
            fires.push(new Fire(bx));
            for (let p = 0; p < 15; p++) parts.push(new Part(bx + Math.sin(p*3)*20, by + Math.cos(p*2)*15, '#ff4400'));
            for (let p = 0; p < 8; p++) parts.push(new Part(bx + Math.sin(p*5)*12, by - 20 + Math.cos(p*3)*10, '#ffcc00'));
            // Línea de humo del misil
            for (let p = 0; p < 5; p++) parts.push(new Part(bx + Math.sin(p*2)*6 - 10 + Math.random()*20, this.y + (by - this.y) * (p/5), '#888'));
            // Daño en área
            enms.forEach(e => {
                if (Math.abs(e.x - bx) < 90) {
                    e.hp -= 10;
                    e.x += (e.x - bx) * 0.3;
                    if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                }
    ...
    draw() {
        ...
        ctx.save();
        let sx = this.x - camX;
        // Láser designador al objetivo
        if (this.target && this.target.hp > 0 && this.target.x > camX - 100 && this.target.x < camX + cvs.width + 100) {
            let tx = this.target.x - camX, ty = this.target.y;
            // Láser rojo pulsante
            ctx.strokeStyle = `rgba(255, 50, 50, ${0.4 + Math.sin(gframe * 0.15) * 0.3})`;
            ctx.lineWidth = 1.5;
            ctx.setLineDash([6, 6]);
            ctx.beginPath(); ctx.moveTo(sx, this.y); ctx.lineTo(tx, ty); ctx.stroke();
            ctx.setLineDash([]);
            // Círculo de puntería en el objetivo
            ctx.strokeStyle = `rgba(255, 50, 50, ${0.6 + Math.sin(gframe * 0.1) * 0.3})`;
            ctx.lineWidth = 1.5;
            ctx.beginPath(); ctx.arc(tx, ty, 8 + Math.sin(gframe * 0.12) * 3, 0, 7); ctx.stroke();
            ctx.beginPath(); ctx.arc(tx, ty, 16 + Math.cos(gframe * 0.08) * 4, 0, 7); ctx.stroke();
            // Crosshair
            ctx.strokeStyle = `rgba(255, 50, 50, ${0.3 + Math.sin(gframe * 0.1) * 0.2})`;
            ctx.lineWidth = 1;
            ctx.beginPath(); ctx.moveTo(tx - 12, ty); ctx.lineTo(tx - 4, ty);
            ctx.moveTo(tx + 4, ty); ctx.lineTo(tx + 12, ty);
            ctx.moveTo(tx, ty - 12); ctx.lineTo(tx, ty - 4);
            ctx.moveTo(tx, ty + 4); ctx.lineTo(tx, ty + 12);
            ctx.stroke();
        }
        // Cuerpo del dron Reaper
        ctx.translate(sx, this.y);
        // Alas en flecha
        ctx.fillStyle = '#1a1a2e';
        ctx.beginPath(); ctx.moveTo(-5, 0); ctx.lineTo(-35, 8); ctx.lineTo(-35, 4); ctx.lineTo(-5, -2); ctx.fill();
        ctx.beginPath(); ctx.moveTo(5, 0); ctx.lineTo(35, 8); ctx.lineTo(35, 4); ctx.lineTo(5, -2); ctx.fill();
        // Cuerpo central
        ctx.fillStyle = '#2a2a3e';
        ctx.beginPath(); ctx.ellipse(0, 3, 8, 4, 0, 0, 7); ctx.fill();
        // Nariz
        ctx.fillStyle = '#3a3a5e';
        ctx.beginPath(); ctx.arc(0, 0, 4, 0, 7); ctx.fill();
        // Ojo/cámara IR (rojo pulsante)
        ctx.fillStyle = `rgba(255, 0, 0, ${0.5 + Math.sin(gframe * 0.15) * 0.3})`;
        ...
        // Hélice trasera
        ctx.fillStyle = `rgba(200,200,200,${0.3 + Math.sin(gframe * 0.4) * 0.2})`;
        ctx.fillRect(-8, 10, 16, 1.5);
        // Antena
        ctx.strokeStyle = '#555'; ctx.lineWidth = 0.8;
        ctx.beginPath(); ctx.moveTo(3, 0); ctx.lineTo(8, -6); ctx.stroke();
        ctx.fillStyle = '#f00'; ctx.beginPath(); ctx.arc(8, -7, 1.5, 0, 7); ctx.fill();
        ctx.restore();
    }
}
...
class Player {
    constructor() { this.w = 30; this.h = 55; this.x = 100; this.y = cvs.height - 100; this.vx = 0; this.vy = 0; this.jmp = false; this.f = 1; this.st = 'idle'; this.hp = 100; this.inv = 0; this.defense = 0; this.damageMod = 0; this._invulnerable = false; this._cargando = false; }
    draw() {
        ctx.save(); ctx.translate(this.x + this.w / 2 - camX, this.y + this.h / 2);
        if (this.f === -1) ctx.scale(-1, 1);
        if (this.inv % 10 > 5) ctx.globalAlpha = 0.5;
        // Efecto de velocidad para Perro en carga
        if (this._cargando) {
            ctx.shadowBlur = 25; ctx.shadowColor = '#ff6600';
        }
        let b = (this.st === 'walk' || this._cargando) ? Math.sin(gframe * 0.3) * 4 : 0;
        renderChar(ctx, selectedChar, b);
        ctx.restore(); ctx.globalAlpha = 1.0; ctx.shadowBlur = 0;
    }
    update() {
        if (this.inv > 0) this.inv--;
        // Perro en carga: movimiento forzado hacia adelante
        if (this._cargando) {
            this.vx = this.f * 8;
            this.st = 'walk';
        } else {
    ...
    dmg(a) { if (this.inv > 0) return; if (this._invulnerable) { for (let i = 0; i < 10; i++) parts.push(new Part(this.x + 15 + Math.sin(i*3)*20, this.y + 25 + Math.cos(i*2)*15, '#ffd700')); return; } let finalDmg = Math.max(1, a - this.defense); this.hp -= finalDmg; this.inv = 60; psnd('hit'); for (let i = 0; i < 15; i++) parts.push(new Part(this.x + 15, this.y + 25, '#f00')); updateUI(); if (this.hp <= 0) { lives--; abilActive = false; abilDur = 0; abilCd = 0; this._cargando = false; this._invulnerable = false; this.defense = 0; this.damageMod = 0; enms.forEach(e => { if (e._origSp) { e.sp = e._origSp; delete e._origSp; } delete e._marked; delete e._markedDEA; delete e._hackedByJoe; delete e._hacked; }); if (window._fogataEspiritus) window._fogataEspiritus = []; if (lives > 0) { this.hp = 100; this.x = 100; this.y = cvs.height - 100; this.vy = 0; camX = 0; } else { gover = true; } } }
}
...
window.saveGameScore = function() {
    ...
    fetch('api_leaderboard.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
    .then(() => { document.getElementById('save-score-section').classList.add('d-none'); loadLeaderboard(); });
}

function loadLeaderboard() {
    fetch('api_leaderboard.php').then(r => r.json()).then(data => {
...
let enms = [], itms = [], mlts = [], fires = [], abuls = [], ebuls = [], dynas = [], birds = [], llamas = [], obss = [], helis = [], gases = [], gasps = [], radioWaves = [], drones = [], teargas = [], fogatas = [], murosTacticos = [], cocaLeaves = [];
...
function renderChar(x, type, b) {
    ...
    if (type.includes('pm')) {
        ...
        // Cap. LARI - militar boliviano, uniforme de gala, boina, condecoraciones
        // Piernas
    ...
    // EFECTOS VISUALES ESPECIALES POR HABILIDAD
    if (gAura) {
        if (type.includes('policia')) {
            // Macana policial + escudo protector azul
            x.save();
            let pulse = Math.sin(gframe * 0.08) * 0.1 + 0.2;
            // Macana (porra) en la mano derecha
            let macAng = Math.sin(gframe * 0.1) * 0.4 - 0.3;
            x.save();
            x.translate(24, 8);
            x.rotate(macAng);
            x.fillStyle = '#4a2800'; x.fillRect(-2, -20, 4, 22);
            x.fillStyle = '#333'; x.fillRect(-3, 2, 6, 4);
            x.fillStyle = '#666'; x.fillRect(-3, -20, 6, 3);
            x.strokeStyle = `rgba(200, 200, 200, ${0.2 + pulse})`;
            x.lineWidth = 1.5;
            for (let i = 0; i < 3; i++) {
                let sy = -16 + i * 8;
                x.strokeRect(-2.5, sy, 5, 4);
            }
            x.restore();
            // Escudo protector azul
            x.strokeStyle = `rgba(68, 136, 255, ${pulse})`;
            x.lineWidth = 3;
            x.beginPath(); x.arc(0, 0, 35 + Math.sin(gframe * 0.06) * 5, 0, 7); x.stroke();
            x.strokeStyle = `rgba(68, 136, 255, ${pulse * 0.5})`;
            x.lineWidth = 1.5;
            x.beginPath(); x.arc(0, 0, 45 + Math.sin(gframe * 0.05 + 1) * 5, 0, 7); x.stroke();
            x.fillStyle = `rgba(68, 136, 255, ${pulse * 0.1})`;
            x.beginPath(); x.arc(0, 0, 35, 0, 7); x.fill();
            x.restore();
        } else if (type.includes('dea')) {
            // Feed de dron Reaper — retícula termal + escáner
            x.save();
            let pulse = Math.sin(gframe * 0.1) * 0.15 + 0.3;
            // Fondo térmico (infrarrojo)
            let grad = x.createRadialGradient(0, 0, 5, 0, 0, 40);
            grad.addColorStop(0, `rgba(255, 50, 50, ${pulse * 0.12})`);
            grad.addColorStop(0.5, `rgba(100, 50, 255, ${pulse * 0.08})`);
            grad.addColorStop(1, `rgba(0, 0, 0, 0)`);
            x.fillStyle = grad;
            x.fillRect(-30, -30, 60, 60);
            // Mira de designación láser (círculos concéntricos)
            x.strokeStyle = `rgba(255, 50, 50, ${pulse})`;
            x.lineWidth = 1.5;
            x.beginPath(); x.arc(0, 0, 12 + Math.sin(gframe * 0.08) * 2, 0, 7); x.stroke();
            x.strokeStyle = `rgba(255, 100, 50, ${pulse * 0.5})`;
            x.lineWidth = 1;
            x.beginPath(); x.arc(0, 0, 24 + Math.cos(gframe * 0.06) * 3, 0, 7); x.stroke();
            // Crosshair
            x.strokeStyle = `rgba(255, 50, 50, ${pulse * 0.8})`;
            x.lineWidth = 1;
            x.beginPath(); x.moveTo(-18, 0); x.lineTo(-6, 0);
            x.moveTo(6, 0); x.lineTo(18, 0);
            x.moveTo(0, -18); x.lineTo(0, -6);
            x.moveTo(0, 6); x.lineTo(0, 18);
            x.stroke();
            // Texto HUD del dron
            x.fillStyle = `rgba(255, 50, 50, ${pulse * 0.6})`;
            x.font = "bold 4px monospace";
            x.fillText("REAPER-1", -18, -28);
            x.fillText("IR-ACTIVE", 10, 28);
            // Escáner térmico (línea barriendo)
            let scanX = Math.sin(gframe * 0.04) * 24;
            x.fillStyle = `rgba(255, 100, 100, ${0.2 + Math.sin(gframe * 0.1) * 0.1})`;
            x.fillRect(scanX - 1, -24, 2, 48);
            x.restore();
        } else if (type.includes('jara')) {
            // Ondas de sonido del grito de euforia
            x.save();
            let pulse = Math.sin(gframe * 0.1) * 0.15 + 0.3;
            // Anillos de sonido expansivos
            for (let i = 0; i < 3; i++) {
                let phase = gframe * 0.05 + i * 2.1;
                let r = 20 + (phase % 7) * 6;
                let op = Math.max(0, 0.5 - (phase % 7) * 0.07);
                x.strokeStyle = `rgba(255, 200, 50, ${op * pulse})`;
                x.lineWidth = 2 + Math.sin(gframe * 0.08 + i) * 0.5;
                x.beginPath(); x.arc(0, 0, r, 0, 7); x.stroke();
                x.strokeStyle = `rgba(255, 150, 0, ${op * pulse * 0.4})`;
                x.lineWidth = 1;
                x.beginPath(); x.arc(0, 0, r + 4, 0, 7); x.stroke();
            }
            // Distorsión de voz (líneas de frecuencia)
            for (let i = 0; i < 8; i++) {
                let ang = i * 0.8 + gframe * 0.02;
                let dist = 28 + Math.sin(gframe * 0.06 + i * 2) * 6;
                let amp = Math.abs(Math.sin(gframe * 0.1 + i)) * 6;
                let x1 = Math.cos(ang) * dist;
                let y1 = Math.sin(ang) * dist;
                let x2 = Math.cos(ang) * (dist + 4 + amp);
                let y2 = Math.sin(ang) * (dist + 4 + amp);
                x.strokeStyle = `rgba(255, 200, 50, ${pulse * 0.4})`;
                x.lineWidth = 1.5;
                x.beginPath(); x.moveTo(x1, y1); x.lineTo(x2, y2); x.stroke();
            }
            // Fulgor central
            x.fillStyle = `rgba(255, 200, 50, ${pulse * 0.15})`;
            x.beginPath(); x.arc(0, 0, 12 + Math.sin(gframe * 0.08) * 3, 0, 7); x.fill();
            x.restore();
        } else if (type.includes('cholita')) {
            // Espíritus de fuego danzando alrededor
            x.save();
            let t = Date.now() * 0.003;
            for (let i = 0; i < 4; i++) {
                let angle = t + i * Math.PI / 2;
                let dist = 30 + Math.sin(t * 2 + i) * 8;
                let fx = Math.cos(angle) * dist;
                let fy = Math.sin(angle) * dist;
                let size = 5 + Math.sin(t * 3 + i * 2) * 3;
                x.fillStyle = `rgba(255, ${150 + Math.sin(t*4+i)*50}, 0, ${0.4 + Math.sin(t*2+i)*0.2})`;
                x.beginPath(); x.arc(fx, fy - 15, size, 0, 7); x.fill();
                x.fillStyle = `rgba(255, 255, 100, ${0.3 + Math.sin(t*3+i)*0.2})`;
                x.beginPath(); x.arc(fx, fy - 15, size * 0.5, 0, 7); x.fill();
            }
            x.restore();
        } else if (type.includes('poncho')) {
            // TEJIDO SAGRADO - Patrones de aguayo girando (ESPECTACULAR)
            x.save();
            let t = gframe * 0.05;
            let colors = ['#ff0000', '#ffd700', '#00a650', '#0032a0', '#8b0000', '#ffffff'];
            for (let ring = 0; ring < 3; ring++) {
                let segments = 8 - ring * 2;
                let radius = 25 + ring * 15 + Math.sin(t + ring) * 5;
                for (let i = 0; i < segments; i++) {
                    let angle = t * (1 - ring * 0.2) + i * (Math.PI * 2 / segments);
                    let rx = Math.cos(angle) * radius;
                    let ry = Math.sin(angle) * radius * 0.6;
                    x.fillStyle = colors[(i + ring * 2) % colors.length];
                    x.globalAlpha = 0.3 + Math.sin(t + i) * 0.15;
                    x.beginPath();
                    x.moveTo(0, -8);
                    x.lineTo(rx - 4, ry - 12);
                    x.lineTo(rx + 4, ry - 12);
            ...
            // Destello central
            let flash = Math.sin(t * 2) * 0.3 + 0.7;
            x.fillStyle = `rgba(255, 215, 0, ${flash * 0.2})`;
            x.beginPath(); x.arc(0, -8, 20 + Math.sin(t) * 5, 0, 7); x.fill();
            x.globalAlpha = 1;
            x.restore();
        } else if (type.includes('evo')) {
            // Ondas de voz doradas
            x.save();
            let t = gframe * 0.06;
            for (let i = 0; i < 3; i++) {
                let radius = 20 + i * 15 + Math.sin(t + i * 2) * 5;
                let pulse = 0.15 + Math.sin(t * 1.5 + i) * 0.1;
                x.strokeStyle = `rgba(255, 215, 0, ${pulse})`;
                x.lineWidth = 2 - i * 0.5;
                x.beginPath();
                for (let a = 0; a < 12; a++) {
                    let angle = a * Math.PI / 6 + t * 0.3;
                    let r = radius + Math.sin(angle * 3 + t) * 5;
                    let px = Math.cos(angle) * r;
                    let py = Math.sin(angle) * r * 0.5 - 8;
            ...
            // Aura de fuego + explosiones
            x.save();
            let t = gframe * 0.08;
            // Llamarada naranja
            let flameSize = 15 + Math.sin(t * 2) * 5;
            let grad = x.createRadialGradient(0, -5, 2, 0, -5, flameSize + 15);
            grad.addColorStop(0, 'rgba(255, 200, 50, 0.4)');
            grad.addColorStop(0.5, 'rgba(255, 100, 0, 0.2)');
            grad.addColorStop(1, 'rgba(255, 0, 0, 0)');
            x.fillStyle = grad;
            x.fillRect(-30, -40, 60, 60);
            // Chispas
            for (let i = 0; i < 6; i++) {
                let angle = t + i * 1.05;
                let dist = 20 + Math.sin(t * 2 + i * 3) * 8;
                let sx = Math.cos(angle) * dist;
                let sy = Math.sin(angle) * dist - 5;
                x.fillStyle = `rgba(255, ${150 + Math.sin(t*3+i)*100}, 0, ${0.5 + Math.sin(t*2+i)*0.3})`;
                x.fillRect(sx - 1.5, sy - 1.5, 3, 3);
            }
            x.restore();
        }
    }
...
function init(lvl = 1) { 
    currentLevel = lvl; enms = []; itms = []; mlts = []; fires = []; abuls = []; ebuls = []; dynas = []; helis = []; gases = []; gasps = []; radioWaves = []; drones = []; teargas = []; fogatas = []; murosTacticos = []; cocaLeaves = []; camX = 0; plyr.x = 100; gwin = false; gover = false; evoTimer = 0; evoSpecialTimer = 0; abilCd = 0; abilDur = 0; abilActive = false; plyr._invulnerable = false; plyr._cargando = false; plyr.damageMod = 0; if (window._fogataEspiritus) window._fogataEspiritus = [];
...
function dScene() {
    ...
    // ENTIDADES
    birds.forEach(b => b.draw()); helis.forEach(h => h.draw()); drones.forEach(d => d.draw()); gases.forEach(g => g.draw()); gasps.forEach(g => g.draw()); teargas.forEach(t => t.draw()); fogatas.forEach(f => f.draw()); murosTacticos.forEach(m => m.draw());
    llamas.forEach(l => l.draw()); obss.forEach(o => o.draw());
    itms.forEach(i => { if (i.x > camX - 80 && i.x < camX + cvs.width + 80) i.draw(); });
    enms.forEach(e => {
        if (e.x > camX - 100 && e.x < camX + cvs.width + 100) {
            e.draw();
            // Indicador de enemigo marcado (PM visor táctico)
            if (e._marked) {
                ctx.save();
                ctx.strokeStyle = `rgba(255, 50, 50, ${0.3 + Math.sin(gframe * 0.1) * 0.2})`;
                ctx.lineWidth = 2;
                ctx.setLineDash([3, 3]);
                ctx.strokeRect(e.x - camX - 18, e.y - 30, 36, 60);
                ctx.setLineDash([]);
                ctx.fillStyle = '#ff4444';
                ctx.font = "bold 7px Arial";
                ctx.fillText('◉', e.x - camX - 3, e.y - 35);
                ctx.restore();
            }
            // Indicador DEA
            if (e._markedDEA) {
                ctx.save();
                let pulse = Math.sin(gframe * 0.08) * 0.3 + 0.5;
                ctx.strokeStyle = `rgba(68, 136, 255, ${pulse})`;
                ctx.lineWidth = 1.5;
                ctx.beginPath();
                ctx.arc(e.x - camX, e.y - 2, 22 + Math.sin(gframe * 0.06) * 4, 0, 7);
                ctx.stroke();
                ctx.fillStyle = `rgba(68, 136, 255, ${pulse * 0.2})`;
                ctx.fillStyle = '#4488ff';
                ctx.font = "bold 8px Arial";
                ctx.fillText('🎯', e.x - camX - 8, e.y - 28);
                ctx.restore();
            }
            // Indicador de enemigo hakeado (Joe)
            if (e._hackedByJoe) {
                ctx.save();
                let pulse = Math.sin(gframe * 0.12) * 0.3 + 0.5;
                ctx.fillStyle = `rgba(0, 255, 136, ${pulse * 0.3})`;
                ctx.fillRect(e.x - camX - 20, e.y - 32, 40, 64);
                ctx.strokeStyle = `rgba(0, 255, 136, ${pulse})`;
                ctx.lineWidth = 1;
                ctx.strokeRect(e.x - camX - 20, e.y - 32, 40, 64);
                ctx.fillStyle = '#00ff88';
                ctx.font = "bold 6px monospace";
                ctx.fillText('>HACK', e.x - camX - 14, e.y - 36);
                ctx.restore();
            }
...
function upd() {
    ...
    if (abilActive && abilDur > 0) {
        ...
        if (abilDur <= 0) {
            ...
            if (selectedChar === 'pm') {
                plyr.damageMod -= 1;
                enms.forEach(e => { if (e._marked) { delete e._marked; } if (e._origSp) { e.sp = e._origSp; delete e._origSp; } });
            } else if (selectedChar === 'policia_bol') {
                plyr.defense -= 20;
            } else if (selectedChar === 'dea') {
                // No hace falta limpiar nada — el dron se elimina solo
            } else if (selectedChar === 'jara') {
                // Grito de euforia terminado
            } else if (selectedChar === 'poncho_j') {
                plyr.defense -= 100;
                plyr._invulnerable = false;
            } else if (selectedChar === 'evo_j') {
                ...
                enms.forEach(e => { if (e._origSp) { e.sp = e._origSp; delete e._origSp; } if (e._hackedByJoe) { delete e._hackedByJoe; } });
            } else if (selectedChar === 'cholita') {
                if (window._fogataEspiritus) window._fogataEspiritus = [];
            }
        }
    ...
    murosTacticos.forEach(m => m.update()); murosTacticos = murosTacticos.filter(m => m.a);

    // ESPÍRITUS DE FUEGO (Cholita)
    if (abilActive && selectedChar === 'cholita' && window._fogataEspiritus) {
        window._fogataEspiritus.forEach(fs => {
            fs.timer++;
            if (fs.timer % 60 === 0) {
                // Spawn espíritu que ataca enemigos
                let sx = fs.x + Math.sin(fs.timer*0.1)*40;
                let sy = fs.y - 20;
                for (let p = 0; p < 6; p++) parts.push(new Part(sx + Math.sin(p*3)*10, sy + Math.cos(p*2)*8, '#ffaa00'));
                enms.forEach(e => {
                    if (Math.abs(e.x - sx) < 120) {
                        e.hp -= 2;
                        if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                    }
                });
                // Efecto visual: llama voladora
                for (let fp = 0; fp < 5; fp++) parts.push(new Part(sx + Math.sin(fp*2)*15 + Math.sin(fs.timer*0.2)*20, fs.y - 60 + Math.cos(fp)*15 + Math.sin(fs.timer*0.15)*10, ['#ff0','#f80','#f40','#fff'][fp%4]));
            }
        });
    }

    // ENEMIGOS HACKEADOS se atacan entre sí (Joe)
    if (selectedChar === 'joe' && abilActive) {
        enms.forEach(e1 => {
            if (e1._hackedByJoe && e1.hp > 0) {
                enms.forEach(e2 => {
                    if (e1 !== e2 && e2.hp > 0 && Math.abs(e1.x - e2.x) < 80) {
                        if (gframe % 30 === 0) {
                            e2.hp -= 2;
                            for (let p = 0; p < 3; p++) parts.push(new Part(e2.x + Math.sin(p)*6, e2.y - 10 + Math.cos(p)*6, '#00ff88'));
                            if (e2.hp <= 0) { totalMoney += (e2.t.includes('evo') ? 30 : (e2.t.includes('cob') ? 10 : 5)); e2.x = -2000; updateUI(); }
                        }
                    }
                });
            }
    ...
    // HOJAS DE COCA (Evo - lluvia de coca)
    if (selectedChar === 'evo_j' && abilActive) {
        if (gframe % 8 === 0) {
            let leaf = new CocaLeaf();
            leaf.x = Math.random() * cvs.width;
            leaf.y = -20;
        ...
        cocaLeaves.forEach(l => {
            l.update();
            enms.forEach(e => {
                if (e.hp > 0 && Math.abs(e.x - (l.x + camX)) < 25 && Math.abs(e.y - l.y) < 35) {
                    l.y = cvs.height + 50;
                    e.hp -= 3;
                    for (let p = 0; p < 5; p++) parts.push(new Part(e.x + Math.sin(p*3)*8, e.y - 10 + Math.cos(p*2)*8, '#7acf5a'));
                    if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                    psnd('hit');
                }
            });
        });
        cocaLeaves = cocaLeaves.filter(l => l.y < cvs.height + 30);
    }
    // GRITO DE EUFORIA (LARI) — pulsos periódicos
    if (selectedChar === 'jara' && abilActive) {
        if (gframe % 40 === 0) {
            enms.forEach(e => {
                if (e.hp > 0 && Math.abs(e.x - plyr.x) < 300) {
                    e.hp -= 5;
                    e.x += (e.x - plyr.x) * 0.3;
                    for (let p = 0; p < 4; p++) parts.push(new Part(e.x + Math.sin(p*3)*8, e.y + Math.cos(p*2)*8, '#ffdd44'));
                    if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); }
                }
            });
            radioWaves.push(new RadioWave(plyr.x, plyr.y + 25));
            for (let p = 0; p < 12; p++) parts.push(new Part(plyr.x + 15 + Math.sin(p*5)*25, plyr.y + 25 + Math.cos(p*3)*15, '#ffdd44'));
        }
    }
    ...
    // FUEGO — daño a jugador siempre; a enemigos solo si es fuego del jugador
    fires.forEach(f => {
        f.update();
    ...
    abuls.forEach(b => { b.update(); enms.forEach(e => { if (b.a && e.x > camX - 50 && e.x < camX + cvs.width + 50 && Math.abs(b.x - e.x) < 40 && Math.abs(b.y - e.y) < 60) { b.a = false; let dmg = 1 + plyr.damageMod; if (e._markedDEA) dmg = Math.floor(dmg * 1.5); if (e._marked) dmg += 1; e.hp -= dmg; psnd('hit'); for (let p = 0; p < 3; p++) parts.push(new Part(e.x + Math.sin(p)*6, e.y - 10 + Math.cos(p)*6, e._markedDEA ? '#4488ff' : (e._marked ? '#ff4444' : '#ffee00'))); if (e.hp <= 0) { totalMoney += (e.t.includes('evo') ? 30 : (e.t.includes('cob') ? 10 : 5)); e.x = -2000; updateUI(); } } }); }); abuls = abuls.filter(b => b.a);
    ...
    // DINAMITA — daño solo al jugador al explotar
    dynas.forEach(d => {
        const wasAlive = d.a;
        d.update();
        if (wasAlive && !d.a) {
